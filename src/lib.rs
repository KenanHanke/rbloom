...
impl Bloom {
    ...
    #[classmethod]
    fn load(_cls: &Bound<'_, PyType>, filepath: &str, hash_func: &Bound<'_, PyAny>) -> PyResult<Bloom> {
    ...
    #[classmethod]
    fn load_bytes(_cls: &Bound<'_, PyType>, bytes: &[u8], hash_func: &Bound<'_, PyAny>) -> PyResult<Bloom> {
        // check that the hash_func is callable
        if !hash_func.is_callable() {
            return Err(PyTypeError::new_err("hash_func must be callable"));
        }
